<!--
  MapPolyline.svelte — Zero-DOM polyline component.
  Creates an L.polyline on the parent map/group via context.
-->
<script lang="ts">
  ...
  let layer: L.Polyline | undefined;
  ...
  $effect(() => {
    ...
    layer = untrack(() => {
      const line = L.polyline(toLatLngs(positions), styleOpts())
