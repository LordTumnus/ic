<script lang="ts">
  ...
  const metric = untrack(() => config?.metric ?? 'total');
  ...
  let numMin = $state(initMode === 'range' ? ((init as NumberFilterValue)?.min ?? null) : null);
  let numMax = $state(initMode === 'range' ? ((init as NumberFilterValue)?.max ?? null) : null);
  ...
  function emit() {
    ...
    const fv: SparklineFilterValue = { metric };
    if (numMin != null) fv.min = numMin;
    if (numMax != null) fv.max = numMax;
    onchange?.((fv.min != null || fv.max != null) ? fv : null);
  }
...
{#if mode === 'range'}
  <div class="ic-tbl-fn__info">{metric === 'relative' ? 'Filters by % change' : 'Filters by last value'}</div>
  <div class="ic-tbl-fn__range">
    <label class="ic-tbl-fn__label">
      Min
      <input
        bind:this={minEl}
        type="number"
        class="ic-tbl-fn__input"
        bind:value={numMin}
        oninput={emit}
        placeholder="—"
      />
