<script lang="ts">
  ...
  import type { NumberFilterValue, ProgressBarConfig } from '$lib/utils/table-utils';
  ...
  const rangeMin = $derived(config?.min ?? 0);
  const rangeMax = $derived(config?.max ?? 100);
  ...
  function emit() {
    ...
    onchange?.((range.min != null || range.max != null) ? range : null);
  }
