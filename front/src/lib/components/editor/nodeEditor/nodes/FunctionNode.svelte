<!--
  FunctionNode.svelte — f(in1,in2,...) combiner block with N inputs → 1 output.
  Displays header with label, expression badge, N input port rows, and one output.
...
<script lang="ts">
  ...
  type FunctionNodeType = Node<FunctionData, 'ic.node.Function'>;

  let { id, data, selected, dragging }: NodeProps<FunctionNodeType> = $props();

  let hovered = $state(false);

  const accentColor = $derived(data.color || 'var(--ic-primary)');
  const minH = $derived(60 + (data.inputs?.length ?? 0) * 24);
</script>
...
>
  ...
  <div class="ic-ne-node__body">
    <div class="ic-ne-node__expression">
      <span class="ic-ne-node__expression-badge">f()</span>
      <InlineEdit
        value={data.expression || ''}
        className="ic-ne-node__expression-edit"
        placeholder="e.g. in1 + in2"
    ...
    <div class="ic-ne-node__ports">
      ...
      {#if (data.inputs?.length ?? 0) === 0 && data.outputs?.[0]}
        {@const outPort = data.outputs[0]}
        <div class="ic-ne-node__port-row">
          <span class="ic-ne-node__port-label ic-ne-node__port-label--right"
            >{outPort.label || outPort.name}</span
