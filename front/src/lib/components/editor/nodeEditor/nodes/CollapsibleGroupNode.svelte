<!--
  CollapsibleGroupNode.svelte — Collapsible subflow container.
  Header with accent stripe, collapse toggle, resize handles, boundary ports.
...
<script lang="ts">
  ...
  type GroupNodeType = Node<GroupData, 'ic.node.CollapsibleGroup'>;

  let { id, data, selected, dragging }: NodeProps<GroupNodeType> = $props();

  let hovered = $state(false);

  const accentColor = $derived(data.accentColor || 'var(--ic-primary)');
