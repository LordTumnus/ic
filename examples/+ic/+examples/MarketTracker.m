...
% shared state (parent workspace — visible to all nested functions)
flashedSelectors = {};
flashTick = 0;

[frame, fig, tbl, searchBar, sizeToggle, titleLabel] = buildUI();
applyStyle();
wireCallbacks();
startTimer();

function [frame, fig, tbl, searchBar, sizeToggle, titleLabel] = buildUI()
    ...
    rootCol = ic.FlexContainer( ...
        "Direction", "column", "Gap", 0, "Padding", 0);
    frame.addChild(rootCol);
    rootCol.css.fill();

    [searchBar, sizeToggle, titleLabel] = buildHeader(rootCol);
    tbl = buildTable(rootCol);
end

function [searchBar, sizeToggle, titleLabel] = buildHeader(parent)
    headerRow = ic.FlexContainer( ...
        "Direction", "row", "Gap", 12, ...
        "Padding", [10 16], "AlignItems", "center");
    parent.addChild(headerRow);

    titleLabel = ic.Label( ...
        "Text", "Market Tracker", ...
        "Variant", "heading", "Size", "xl", "Weight", "bold");
...
function tbl = buildTable(parent)
    tbl = ic.Table( ...
        ...
        "ShowRowNumbers", true, "Size", "md");
    parent.addChild(tbl);
    tbl.css.fill();
    tbl.Columns = defineColumns();
    tbl.Data = generateMarketData();
end

function cols = defineColumns()
    cols = [
...
function applyStyle()
    headerRow = searchBar.Parent; %#ok<NASGU> - used via titleLabel's parent
    titleLabel.css.style("> *", ...
        "color", "#e2e8f0", ...
        "textShadow", "0 0 20px rgba(59,130,246,0.3)");
    searchBar.Parent.css.style("> *", ...
    ...
    flashDown = struct( ...
        ...
        "to",   struct("boxShadow", "inset 0 0 0 transparent"));
    tbl.css.keyframes("flash-up-a",   flashUp);
    tbl.css.keyframes("flash-up-b",   flashUp);
    tbl.css.keyframes("flash-down-a", flashDown);
    tbl.css.keyframes("flash-down-b", flashDown);
end
...
function onSearchChanged(e)
    tags = string(e.Data.value);
    tags(tags == "" | ismissing(tags)) = [];
    if isempty(tags)
        tbl.Filters = struct();
    else
        tbl.Filters = struct("Symbol", strjoin(tags, " "));
    end
end

function onSelectionChanged(e)
    sel = e.Data.selection;
    if ~strcmp(sel.type, "row") || isempty(sel.value), return; end
    rowIdx = sel.value(1);
    ...
    frame.addChild(ic.Toast( ...
        "Value", msg, "Variant", "info", "Duration", 3, "Position", "bottom"));
end
...
function tickPrices()
    n = height(tbl.Data);
    oldPrices = tbl.Data.Price;

    pctChange = 0.005 * randn(n, 1);
    newPrices = oldPrices .* (1 + pctChange);
    newChange = tbl.Data.Change24h + pctChange * 100 * 0.3;
    newSpark = tbl.Data.Sparkline;
    for i = 1:n
        newSpark{i} = [newSpark{i}(2:end), newPrices(i)];
    end
    ...
    direction = sign(newPrices - oldPrices);
    editArgs = cell(1, 9 * n);
    upRows = {};
    downRows = {};
    for i = 1:n
        base = (i - 1) * 9;
        editArgs{base+1} = i;  editArgs{base+2} = "Price";     editArgs{base+3} = newPrices(i);
        editArgs{base+4} = i;  editArgs{base+5} = "Change24h"; editArgs{base+6} = newChange(i);
        editArgs{base+7} = i;  editArgs{base+8} = "Sparkline"; editArgs{base+9} = newSpark{i};
    ...
    tbl.editCell(editArgs{:});

    flashTick = flashTick + 1;
    if mod(flashTick, 2) == 0, suffix = "-a"; else, suffix = "-b"; end
...
Sparkline = cell(n, 1);
for i = 1:n
    Sparkline{i} = Price(i) * cumprod(1 + 0.02 * randn(1, 7));
end
...
function files = downloadLogos(symbols, brandColors) %#ok<INUSD>
...
for i = 1:n
    ...
    if ~downloaded
        ...
        svg = sprintf(['<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">' ...
            ...
            'font-weight="700" font-size="18" fill="white">%s</text></svg>'], letter);
